...
# Plot the trajectories
sigma = 15
ax.plot(gaussian_filter1d(old_traj[start_idx:end_idx,0], sigma=sigma), gaussian_filter1d(old_traj[start_idx:end_idx,1], sigma=sigma), gaussian_filter1d(old_traj[start_idx:end_idx,2], sigma=sigma), label='Demonstration', color='black', linestyle='--', linewidth=2)
for n in range(len(transported_trajs)):
    ax.plot(gaussian_filter1d(transported_trajs[n][start_idx:end_idx,0], sigma=sigma), gaussian_filter1d(transported_trajs[n][start_idx:end_idx,1], sigma=sigma),gaussian_filter1d(transported_trajs[n][start_idx:end_idx,2], sigma=sigma), color=colors[n])
...
# for n in range(len(converted_targets_dists)):
    ax.scatter3D(converted_targets_dists_proj[n][0,0], converted_targets_dists_proj[n][0,1],0.05,marker="*", s=120, color=colors[n])
    ax.scatter3D(converted_targets_dists_proj[n][1,0], converted_targets_dists_proj[n][1,1],0.05,marker="$x$", s=100, color=colors[n])
# ax.scatter3D(original_target_distribution[0,0], original_target_distribution[0,1],original_target_distribution[0,2],marker="*", s=100)
...
plt.tight_layout()
plt.savefig("/media/mariano/Windows2/phd/hybrid_arxiv_paper/example_plots/pull_3d_new.pdf")
plt.savefig("/media/mariano/Windows2/phd/hybrid_arxiv_paper/example_plots/pull_3d_new.png")
...
# Plot the trajectories
sigma = 15
demo_shift_x, demo_shift_y, demo_shift_z = source_distribution[centering_marker]
ax.plot(gaussian_filter1d(old_traj[start_idx:end_idx,0]-demo_shift_x, sigma=sigma), gaussian_filter1d(old_traj[start_idx:end_idx,1]-demo_shift_y, sigma=sigma), gaussian_filter1d(old_traj[start_idx:end_idx,2]-demo_shift_z, sigma=sigma), label='Demonstration', color='black', linestyle='--', linewidth=3, zorder=5)
demo_last_z = old_traj[-1,2] - demo_shift_z
for n in range(len(transported_trajs)):
    shift_x, shift_y, _ = converted_targets_dists_proj[n][centering_marker]
    shift_z = transported_trajs[n][-1,2] - demo_last_z
    ax.plot(gaussian_filter1d(transported_trajs[n][start_idx:end_idx,0]-shift_x, sigma=sigma), gaussian_filter1d(transported_trajs[n][start_idx:end_idx,1]-shift_y, sigma=sigma),gaussian_filter1d(transported_trajs[n][start_idx:end_idx,2]-shift_z, sigma=sigma), color=colors[n], lw=3)

# ax.scatter3D(source_distribution[0,0]-source_distribution[1,0], source_distribution[0,1]-source_distribution[1,1],0.025,marker="$x$", s=120, color='black')
# ax.scatter3D(transport.source_distribution[1,0], transport.source_distribution[1,1],transport.source_distribution[1,2],marker="$1$", s=100)
# ax.scatter3D(original_source_distribution[0,0], original_source_distribution[0,1],original_source_distribution[0,2],marker="*", s=100)
...
for n in range(len(converted_targets_dists)):
    shift_x, shift_y, _ = converted_targets_dists_proj[n][centering_marker]
    ax.scatter3D(converted_targets_dists_proj[n][0,0]-shift_x, converted_targets_dists_proj[n][0,1]-shift_y,0.025,marker="x", s=200, linewidths=3, color=colors[n])
...
# ax.scatter3D(gpt.target_distribution[2,0], gpt.target_distribution[2,1],gpt.target_distribution[2,2],marker="$3$", s=100)
# ax.scatter3D(0, 0, 0.025, marker='x', color='black', label='Initial object locations', s=100)
ax.scatter3D([], [], [], marker='x', color='black', label='Goal object location', s=200, linewidths=3)
ax.view_init(elev=25, azim=-45)  # 30° from horizontal, 45° around z-axis
# Customize the plot
# ax.set_title('Transported stacking trajectories')
ax.set_xlabel(r'$x - t_x^1$ [m]', labelpad=15)
...
def add_image_marker(ax, x, y, img, zoom):
    ...
    ax.add_artist(ab)
img_start = mpimg.imread("/home/mariano/phd_code/red_cylinder2.png")
add_image_marker(ax, -0.045, -0.04, img_start, 0.2)
ax.legend()
legend = ax.get_legend()
handles, labels = legend.legend_handles, [text.get_text() for text in legend.texts]
...
# --- now drop the PNGs ------------------------------------------------
for n in range(len(converted_targets_dists)):
    gx, gy = converted_targets_dists_proj[n][0][:-1]      # goal
    sx, sy = converted_targets_dists_proj[n][1][:-1]      # start
    # add_image_marker(ax, gx, gy, img_goal, ZOOM-0.01)
    ax.scatter(gx, gy, marker='x', s=400, linewidths=5, color=colors[n])
    add_image_marker(ax, sx, sy, img_start, (ZOOM+0.08))
    start = converted_targets_dists_proj[n][1][:-1]
    goal = converted_targets_dists_proj[n][0][:-1]
    ...
    ax.annotate('', 
                xy=goal_arrow, 
                xytext=start[:2],
                arrowprops=dict(arrowstyle="->,head_length=0.6,head_width=0.3", lw=5, color=colors[n]), zorder=10)
...
start_handle = object()
ax.scatter([], [], marker='x', color='black', label='Goal pull locations', s=200, linewidths=3)
ax.legend()
legend = ax.get_legend()
handles, labels = legend.legend_handles, [text.get_text() for text in legend.texts]
...
ax.legend(handles, labels,
          ...
          frameon=True, loc="upper left", bbox_to_anchor=(0.05, 1))  # pick a loc you like

plt.tight_layout()
plt.savefig(f"/media/mariano/Windows2/phd/hybrid_arxiv_paper/example_plots/pull_2d_new.pdf")
plt.savefig(f"/media/mariano/Windows2/phd/hybrid_arxiv_paper/example_plots/pull_2d_new.png")
plt.show()
plt.close()
#%%
img = img_start
...
imgs = [
    [mpimg.imread(f"/media/mariano/Windows2/phd/hybrid_arxiv_paper/frames_nb/stack1_frames/frame{k}.png") for k in range(2,6)],      # column 0: 4 RGBA images
    [mpimg.imread(f"/media/mariano/Windows2/phd/hybrid_arxiv_paper/frames_nb/stack2_frames/frame{k}.png") for k in range(2,6)],      # column 1
    [mpimg.imread(f"/media/mariano/Windows2/phd/hybrid_arxiv_paper/frames_nb/stacks_left_right2/frame{k}.png") for k in range(2,6)],      # column 2
]
...
plt.savefig("/media/mariano/Windows2/phd/hybrid_arxiv_paper/example_plots/full_figure_pull.pdf")
#%%
import matplotlib.pyplot as plt
from matplotlib.patches import Rectangle
import numpy as np      # only for dummy data
...
fig        = plt.figure(figsize=(9, 6), constrained_layout=True, dpi=300)
subfig_L, subfig_R = fig.subfigures(
    1, 2, width_ratios=[2, 2],  # 1⁄3  :  2⁄3
)

# ---–  Level 2-L: two rows on the left  –---
axs_left = subfig_L.subplots(
    2, 1, sharex=False, sharey=False, height_ratios=[6, 5]
)
img_top = mpimg.imread("/media/mariano/Windows2/phd/hybrid_arxiv_paper/example_plots/pull_3d_new.png")
img_top = img_top[100:-100, 200:-500]
axs_left[0].imshow(img_top)
axs_left[0].set_axis_off()

img_bottom = mpimg.imread("/media/mariano/Windows2/phd/hybrid_arxiv_paper/example_plots/pull_2d_new.png")
axs_left[1].imshow(img_bottom[50:-50,:])
axs_left[1].set_axis_off()
# axs_left[1].set_title("Bottom plot")
...
fig.text(
    0.04, 0.98, r"$A$", fontsize=16, fontweight='bold', va='top', ha='left'
)
fig.text(
    0.04, 0.48, r"$B$", fontsize=16, fontweight='bold', va='top', ha='left'
)


# --- Right: 3 subfigures (columns), each with 4 stacked image axes ---
subfigs_cols = subfig_R.subfigures(1, 2, wspace=0.05)

imgs = [
    [mpimg.imread(f"/media/mariano/Windows2/phd/hybrid_arxiv_paper/frames_nb/push1/frame{k}.png") for k in range(2,6)],
    [mpimg.imread(f"/media/mariano/Windows2/phd/hybrid_arxiv_paper/frames_nb/push2/frame{k}.png") for k in range(2,6)],
...
for i, (subfig_col, imgset, label) in enumerate(zip(subfigs_cols, imgs, column_labels)):
    ...
    for row, ax in enumerate(axs):
        ...
        timestamp_seconds = seconds[row]  # or however you want to compute time
        ax.text(
            0.02, 0.94,  # x, y in axes coords
